...
from proofloop_core.context.io import sha256_file as _sha256_file, read_json as _read_json_object_from_io
...
def _validate_tree(
    ...
    actual_files: set[str] = {
        f.relative_to(bundle_root).as_posix()
...
def _read_json_object(path: Path) -> dict[str, Any]:
    try:
        return _read_json_object_from_io(path)  # type: ignore[return-value]
