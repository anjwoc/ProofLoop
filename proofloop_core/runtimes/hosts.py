...
from proofloop_core.runtimes.runtime import CONTROLLER_ROUTES, RuntimeRegistry, runtime_spec
...
def probe(host: str) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "path": executable,
        "version": None,
        "modelRoutingStatus": "ROLE_ROUTING_ONLY" if config["mode"] == "ROLE_ROUTING_ONLY" else ("CONFIGURED_UNPROVEN" if config["mode"] == "NATIVE_MODEL_ROUTING" else "ROLE_ROUTING_ONLY"),
...
_AGENT_INSTRUCTIONS = {
    ...
    "classifier_fast": """Act only as ProofLoop's intent classifier. You output only valid JSON according to the schema provided in the prompt. You classify user requests into categories like mutate, investigate, or plan based on the rules.""",
}
...
def render_codex_agent(role: str) -> str:
    ...
    description = {
        ...
        "classifier_fast": "Fast JSON-only intent classifier.",
    }[role]
    return (
...
def role_from_agent_type(value: str | None) -> str | None:
    ...
    for role in ("planner_deep", "explorer_fast", "implementer_fast", "implementer_recovery", "reviewer_deep", "classifier_fast"):
