...
def decide_next(attempts: list[dict[str, Any]], max_fast_attempts: int = 2, max_recovery_attempts: int = 1) -> dict[str, Any]:
    ...
    if classification in {"DESIGN_CONFLICT", "SPEC_AMBIGUITY", "CONTRACT_CHANGE"}:
    ...
    repeats = sum(1 for item in fast if fingerprint and item.get("failureFingerprint") == fingerprint)

    if latest.get("role") == "implementer_fast":
        if repeats >= 2 or len(fast) >= max_fast_attempts:
