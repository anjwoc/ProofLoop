...
def _default_prompt(role: str, task_path: str | None, run_dir: Path) -> str:
    role_text = {
        ...
        "implementer_recovery": "Fix the root cause using exact failure evidence. Do not broaden the task contract.",
        "reviewer_deep": "Independently review the real diff, checks, claims, and simplicity. Do not edit code.",
        "classifier_fast": "Fast JSON-only intent classifier for routing requests.",
        "explorer_fast": "Inspect only relevant repository entry points and return grounded context. Do not edit code.",
    }[role]
    task = f" Read the task brief at {task_path}." if task_path else ""
    return (
        f"Act as ProofLoop role {role}. {role_text}{task} "
        f"Use the evidence run at {run_dir}. Deterministic checks and Truth remain parent-owned."
    )
...
def _write_receipt(
    *,
    ...
    provenance = read_json(root / "run-provenance.json") if (root / "run-provenance.json").is_file() else {}
    ...
    receipt = {
        ...
        "host": host,
        "role": role,
        "executablePath": str(Path(executable).resolve()),
        "executableVersion": _executable_version(executable),
        ...
        "transcriptSha256": _canonical_sha256({
            "stdoutSha256": _sha256_file(stdout_path),
            "stderrSha256": _sha256_file(stderr_path),
        }),
...
def invoke_role(
    ...
    *,
    ...
    roles = {"planner_deep", "explorer_fast", "implementer_fast", "implementer_recovery", "reviewer_deep", "classifier_fast"}
    ...
    if host == "codex":
        ...
        command = [str(executable), *fixed_args, "exec", "--json", "--ephemeral", "--sandbox", sandbox]
        ...
        if mutating_role and (access_mode == "yolo" or os.environ.get("PROOFLOOP_AGY_BYPASS_PERMISSIONS") == "1"):
    ...
    def on_line(stream_name: str, line: str) -> None:
        ...
        try:
            ...
            if emitter is not None:
                emitter.emit(
                    "capability.degraded", phase=phase, message="Host output parser failed.", level="warning", task_id=task_id,
                    data={"role": role, "invocationId": invocation_id, "reasonCode": "OUTPUT_PARSER_FAILED", "error": f"{type(exc).__name__}: {exc}"},
                )
            return
        if emitter is not None and visible_text and not normalized:
            ...
            if not structured:
                emitter.emit(
                    "role.output", phase=phase, message=f"{role} produced {stream_name} output.", task_id=task_id,
                    data={
                        "role": role, "attempt": attempt, "invocationId": invocation_id,
                        "stream": stream_name, "text": visible_text[:_MAX_HOST_OUTPUT_EVENT_CHARS],
                        "truncated": len(visible_text) > _MAX_HOST_OUTPUT_EVENT_CHARS,
                        "stdoutRef": str(call_dir / "stdout.log"), "stderrRef": str(call_dir / "stderr.log"),
                    },
                )
        for item in normalized:
            data = {"role": role, "attempt": attempt, "requestedModel": model, "invocationId": invocation_id, "stream": stream_name, **item.data}
            ...
            if emitter is not None:
                emitter.emit(item.event_type, phase=phase, message=item.message, level=item.level, task_id=task_id, data=data)
                if item.event_type == "role.model_observed":
    ...
    def on_heartbeat(process_id: int, elapsed_seconds: float) -> None:
        if emitter is not None:
            emitter.emit(
                "role.progress", phase=phase, message=f"{role} is still running.", task_id=task_id,
                data={
                    "role": role, "attempt": attempt, "invocationId": invocation_id,
                    "processId": process_id, "elapsedSeconds": round(elapsed_seconds, 1),
                    "stdoutRef": str(call_dir / "stdout.log"), "stderrRef": str(call_dir / "stderr.log"),
                },
            )
    ...
    if process_result.timed_out and process_result.timeout_reason == "INITIAL_OUTPUT_TIMEOUT":
        ...
        failure_reason = f"AGY produced no output within {initial_output_timeout}s."
    if host == "codex":
    ...
    if observed is None and emitter is not None:
        emitter.emit(
            "role.model_observed", phase=phase, message="Requested model could not be observed.", level="warning", task_id=task_id,
            data={
                "role": role, "attempt": attempt, "requestedModel": model,
                "observedModel": None, "evidenceLevel": evidence_level, "invocationId": invocation_id,
            },
        )
        model_event_emitted = True
    ...
    event = {
        ...
        "command": command,
        "processId": process_result.process_id,
    ...
    usage_summary = TokenLedger(root).summarize([*load_invocations(root), {"invocationId": invocation_id}])
    usage = next((item for item in usage_summary["byInvocation"] if item["invocationId"] == invocation_id), None)
    if usage:
        result["usage"] = usage
        write_json(call_dir / "invocation.json", {**event, **result})
    return result
