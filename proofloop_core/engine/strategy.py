...
# ponytail: lightweight regexes for fine-grained risk/direct signals when IntentGate is omitted
...
def classify_request(
    ...
    *,
    ...
    # Derive strategy directly from IntentGateResult if provided (single source of truth)
    if intent_gate_result is not None:
        ik = intent_gate_result.intent_kind.value
        auth = intent_gate_result.authority.value
        clarity = intent_gate_result.clarity.value

        if ik in ("investigate", "answer"):
            strategy = "REPOSITORY_ANALYSIS"
            reason = "read-only analysis intent from intent gate"
        elif auth == "external_side_effect" or clarity == "owner_decision_required" or _HIGH_RISK.search(text):
            strategy = "HIGH_RISK_ENGINEERING"
            reason = "high-risk authority or domain from intent gate"
        elif _DIRECT.search(text) and len(text) < 500:
            strategy = "DIRECT_VERIFIED_CHANGE"
            reason = "bounded local change signal detected"
        else:
            strategy = "PLANNED_IMPLEMENTATION"
            reason = "non-trivial mutation defaults to planned execution"

        return _apply_repository_signals(_decision_for(strategy, (reason,), text), repository_signals)

    # Fallback when IntentGateResult is not passed directly
    # ponytail: Deleted speculative keyword fallback.
