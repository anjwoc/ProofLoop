...
from proofloop_core.strategy import classify_request, reclassify_after_diff
...
class StrategyTest(unittest.TestCase):
    def test_defaults_low_complexity_mutation_to_t1_fast_lane(self) -> None:
        ...
        local = classify_request("Add a helper that returns the configured cache name")
    ...
    def test_small_bounded_change_uses_t0_without_deep_review(self) -> None:
        result = classify_request("README 오타 한 줄만 수정해줘")
    ...
    def test_known_cross_cutting_t2_skips_explorer_but_keeps_planner_and_review(self) -> None:
        result = classify_request("Implement a cache change across API, worker, and database layers")
    ...
    def test_security_signal_is_hard_gate_to_t3(self) -> None:
        result = classify_request("로그인 인증 권한 검사를 한 줄 수정해줘")
    ...
    def test_first_diff_upgrades_underestimated_cross_cutting_work(self) -> None:
        initial = classify_request("README 오타 한 줄만 수정해줘")
    ...
    def test_greenfield_persistent_service_is_t3(self) -> None:
        result = classify_request("Build a new project for a reservation service with persistent state and concurrency")

        self.assertEqual("T3", result.tier)
        self.assertTrue(result.planner_required)
        self.assertTrue(result.reviewer_required)

        greenfield = classify_request("Create a new project that renders a static status page")
